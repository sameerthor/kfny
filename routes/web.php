...
use App\Http\Livewire\Provider\Cases;
...
Route::group(['middleware' => 'auth'], function () {
    ...
    Route::get('litigation', [LigitationController::class, 'livewireView'])->name('litigation');
    ...
    Route::get('cases', Cases::class)->middleware('role:provider')->name('cases');
...
Route::get('/add-role', function () {
    ...
    if (!Role::where('name', 'Attorney')->exists()) {
        Role::create(['name' => 'Attorney']);
    }
    if (!Role::where('name', 'Paralegal')->exists()) {
        Role::create(['name' => 'Paralegal']);
    }
    if (!Role::where('name', 'Clerk')->exists()) {
        Role::create(['name' => 'Clerk']);
    }
    if (!Role::where('name', 'Settlement clerk')->exists()) {
        Role::create(['name' => 'Settlement clerk']);
    }
    if (!Role::where('name', 'Data entry')->exists()) {
